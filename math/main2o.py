...
'''
    ...
    output_data = {
        ...
        "referee_judgment": referee_judgment,  # Added referee's output
...
import re
def calculate_average_scores(results_directory):
    """
    计算并输出指定目录下所有对话结果的裁判平均分。

    Args:
        results_directory (str): 存放结果JSON文件的目录路径。
    """
    accuracy_scores = []
    socratic_scores = []
    rules_scores = []
        ...
        if filename.endswith(".json"):
            ...
            with open(file_path, 'r', encoding='utf-8') as f:
                try:
                    ...
                    if acc_match:
                        accuracy_scores.append(int(acc_match.group(1)))
                    if soc_match:
                        socratic_scores.append(int(soc_match.group(1)))
                    if rul_match:
                        rules_scores.append(int(rul_match.group(1)))
    ...
    if rules_scores:
        avg_rules = sum(rules_scores) / len(rules_scores)
...
if __name__ == "__main__":
    #generate_student()
    # generate_error()
    gpt4_1_path = "result-v5.1/gpt-4.1_results"
    gpt4o_path = "result-v5.1/gpt-4o_results"
    gpt3_5_path = "result-v5.1/gpt-3.5-turbo_results"
    ...
    run_dialogue_full_permutation(student_output, error_output, question_map, "gpt-4.1", gpt4_1_path)
    run_dialogue_full_permutation(student_output, error_output, question_map, "gpt-4o", gpt4o_path)
    run_dialogue_full_permutation(student_output, error_output, question_map, "gpt-3.5-turbo", gpt3_5_path)

    # --- 所有对话运行完毕后，开始自动分析和计算平均分 ---
    print("\n\n==================== 开始分析评估结果 ====================")
    calculate_average_scores(gpt4_1_path)
    calculate_average_scores(gpt4o_path)
    calculate_average_scores(gpt3_5_path)
    print("==================== 分析结束 ====================")
