...
logging.basicConfig(filename='main1.log', level=logging.DEBUG, encoding='utf-8', format='%(asctime)s - %(levelname)s - %(message)s')
...
'''
    ...
    # Sentinel to check for the final confirmation
    zzc_confirmed = False
    ...
    for round_num in range(rounds_max):
        ...
        {prompt_history}
        """
        # This part for student response remains unchanged
        from llm_inf import LLMHandler_inf
        ...
        # This part for teacher response remains unchanged
        client = OpenAI(
    ...
    logging.info(f"[dialogue_id={dialogue_id}] Dialogue finished. Calling referee for evaluation.")
    referee_judgment = "Referee call failed or was not executed."
    try:
        # Define the referee's system prompt here.
        referee_system_prompt = referee_system_prompt = f'''
...
3.  **Adherence to Rules**: Did the teacher fully comply with all constraints : {teacher_system_prompt} 
...
'''
        # Instantiate the referee LLM handler. Using a powerful model like gpt-4o is recommended.
        referee_model = "gpt-4o" 
        handler_referee = LLMHandler_inf(api_key, referee_model)
        ...
        # Use a lower temperature for more consistent evaluation from the referee.
        referee_result_content = handler_referee.get_completion(referee_messages, temperature=0.5)
        
        if referee_result_content:
            referee_judgment = referee_result_content
            logging.info(f"[dialogue_id={dialogue_id}] Referee Judgment Received.")
            logging.debug(f"[dialogue_id={dialogue_id}] Referee Judgment: {referee_judgment}")
        else:
            logging.error(f"[dialogue_id={dialogue_id}] Referee API call failed, returned empty content.")

    except Exception as e:
        logging.error(f"[dialogue_id={dialogue_id}] An error occurred during the referee call: {e}")   
    ...
    output_data = {
        ...
        "referee_judgment": referee_judgment, # Added referee result
    ...
    # Changed file open mode to 'w' to overwrite/create a new file for each dialogue
...
def calculate_and_print_scores(results_directory):
    """
    计算并输出指定目录下所有对话结果的裁判平均分。
    *** 新版本：使用 os.walk 递归搜索所有子目录 ***
    """
    accuracy_scores, socratic_scores, efficiency_scores = [], [], []
    
    if not os.path.exists(results_directory):
        print(f"错误：评估目录 '{results_directory}' 不存在。")
        return

    file_count = 0
    # 使用 os.walk 遍历主目录及其所有子目录
            ...
            if filename.startswith("dialogue_") and filename.endswith(".json"):
                ...
                try:
                    ...
                    if not judgment_text:
                        logging.warning(f"文件 {filename} 中缺少 'referee_judgment'。")
                        continue
                    ...
                    if rul_match: efficiency_scores.append(int(rul_match.group(1)))
                except Exception as e:
    ...
    def print_avg(name, scores):
        ...
        print(f"平均 {name}: {avg:.2f} (基于 {len(scores)} 个评分) | 成功率: {success_rate:.2%}")

    print_avg("准确度 (Accuracy)", accuracy_scores)
    print_avg("苏格拉底方法 (Socratic Method)", socratic_scores)
    print_avg("遵循规则 (Adherence to Rules)", efficiency_scores)
    print("--------------------")
...
def run_dialogue_multithread(error_output, all_question_map, output_root_dir, teacher_model, teacher_api_key):
    ...
    calculate_and_print_scores(output_root_dir)
...
if __name__ == "__main__":
    ...
    teacher_api_key = ""  # 你的DMXAPI key

    # Create dummy files for testing
    if not os.path.exists("new_questions_100.json"):
        with open("new_questions_100.json", "w", encoding="utf-8") as f:
            json.dump([{"index": 1, "question": "What is 2+2?", "answer": "4"}], f)
    if not os.path.exists("errors_index_2_100.json"):
        with open("errors_index_2_100.json", "w", encoding="utf-8") as f:
            json.dump([{"question_index": 1, "error_reason": "I thought 2+2=5"}], f)

    question_map = load_questions_map("new_questions_100.json")
    with open("errors_index_2_100.json", "r", encoding="utf-8") as f:
        error_output = json.load(f)

    # ==================== 修改部分 ====================

    # 1. 在这里定义重复实验的次数
    num_repeats = 5  # <-- 设定重复运行的轮数

    all_model_stats = []
    result_root_dir = os.path.join(os.path.dirname(__file__), "result-v7_100_repeat")

    # 2. 增加一个外层循环，用于重复实验
    for i in range(1, num_repeats + 1):
        print(f"\n=============== 开始第 {i}/{num_repeats} 轮实验 ================")
        
        # 您原有的模型循环
        for model in all_models:
            print(f"--- (第 {i} 轮) 运行模型: {model} ---")
            
            # 3. 修改输出目录，加入轮次信息，防止结果被覆盖
            #    例如生成路径: "result-v7_repeat/repeat_1/model_A_results"
            output_root_dir = os.path.join(result_root_dir, f"repeat_{i}", f"{model}_results")
