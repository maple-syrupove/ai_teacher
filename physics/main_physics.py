...
def generate_error():
    ...
    error_dir = Path(__file__).parent / 'error_physics_new'
    ...
    model = "gpt-4.1"
    system_prompt = """
    **角色:** 你是一位精通物理教育和学生学习心理学的专家。你深刻理解学生在解决物理问题时可能犯的各种错误，并能准确地将这些错误与特定的认知维度（如知识性、逻辑性、策略性、执行性）联系起来。

**任务:** 我将为你提供一个【错误维度表格】和一个具体的【物理问题】。请你严格遵循以下要求，为该问题**固定生成六种不同类型的学生错误**JSON数据。这六种错误必须严格按照**四种单维度错误**和**两种多维度复合错误**的结构进行组织。
...
4.  **精确到点:** "具体错误点"必须明确指出在解决这个特定【物理问题】时，学生在哪一步、哪个公式、哪个物理量上出错了。
...
| -------------------------- | ---------------------------------------------------------- | -------------------------------------------------------- |
| **知识性错误 (Knowledge-Based)** | 因物理知识本身的缺陷、遗忘或理解不深刻导致的错误。         | 概念混淆、规律误记、公式错用、前提条件忽视。             |
| **逻辑性错误 (Logical)** | 因违背物理的严谨性、推理规则和逻辑形式而产生的错误。         | 循环论证、分类讨论不完备、偷换概念、不等价变换。         |
...
def generate_student():
    ...
    背景: 我正在进行一项教育研究项目，需要构建一系列高度逼真的虚拟学生模型。这些模型将用于模拟学生在解答问题（尤其是物理推理问题）时的真实互动场景。我已经通过课堂观察，总结出了学生在语言表达和解题心态上的几个核心维度类型。
    ...
    2.  **字段精确匹配**: 每个JSON对象必须包含以下三个字段：
        ...
        * "学生描述": 一段具体、生动的学生刻画描述。这段描述需要深度融合所选的表达与情感类型，描绘出该学生在**解答一个具体物理问题时（不给出具体例子需要你总结一个通用的心理过程）**的完整思考过程、语言风格和心理状态。描述需包含丰富的细节，展现其内在的逻辑断点或情感挣扎。
    ...
    **表1: 学生表达维度 (Student Expression Dimensions)**
    ...
    | :--- | :--- |
    | **语言模糊与指代不清** | 使用"这个"、"那个"等模糊词汇，解题步骤难以追踪。例如："先把那个...速度算出来...就变成10m/s...然后那个力...等于20N。" |
    | **跳跃式陈述** | 省略关键的中间思维步骤，直接跳到结论。例如："就是...F等于ma，所以F等于20。"（未解释如何从已知条件到F=ma） |
    | **自我修正与不确定性** | 表达犹豫、自我怀疑和修正。例如："先算速度，是10m/s？对，是10m/s。然后...算加速度？好像是...a=2？所以F=20N。" |
    | **"黑话"与非正式语言** | 使用非标准术语。例如："老师教的是'带进去'，所以把10'带进去'就行了。" |
    | **冗余与重复** | 反复陈述同一个步骤或想法。例如："就是先算速度，然后再算速度。对，就是先算速度，然后再算速度。然后加起来。" |
    | **具象化描述** | 依赖于具体事物或动作来描述抽象过程。例如："我有2个小球，又拿来3个小球，我数一下...是5个。"（用此风格描述物理量） |
...
def run_one_dialogue(question, profile, error, dialogue_id, all_question_map=None):
    ...
    # 1. 角色与情景
    角色定位： 您将扮演一名富有同理心、洞察力、且极其耐心的物理教师。您的教学哲学是"诊断先于治疗"。
    ...
    # 4. 终止条件与自我修正
    ## 成功终止 
    "钥匙"： 当您完全确信已经找到学生那个最根本的认知偏差时，请用精准、直接、且不容辩驳的语言，清晰地指出他的错误逻辑起点或物理概念偏差本身。
    ## 自我修正 
    ...
    # 情景：
    你刚刚完成一道物理题，并得出了一个**错误答案**。现在，你将与扮演教师角色的用户进行对话，阐述你的解题过程（禁止将你的错误思维全盘托出）。记住，你是从你的视角出发，真诚地进行表达和捍卫，而非故意捣乱。
    ...
    **表1: 学生表达维度 (Student Expression Dimensions)**
        ...
        **表3：【错误维度表格】**
        | 错误维度 (Error Dimension) | 核心定义 (Core Definition)                                 | 典型表现 (Typical Manifestations)                        |
        | -------------------------- | ---------------------------------------------------------- | -------------------------------------------------------- |
        | **知识性错误 (Knowledge-Based)** | 因物理知识本身的缺陷、遗忘或理解不深刻导致的错误。         | 概念混淆、物理规律误记、公式错用、前提条件忽视。             |
        | **逻辑性错误 (Logical)** | 因违背物理推理规则和逻辑形式而产生的错误。         | 推理链断裂、分类讨论不完备、偷换概念、不等价变换。         |
    ...
    * **目标二：策略性隐藏错误。** 你的第二个任务是让教师难以通过对话快速定位你的根本性错误。你将通过忠实地扮演你的角色来实现这一点。例如，一个自信的学生可能会跳过他认为简单的步骤，一个内向的学生可能会对关键步骤含糊其辞，一个焦虑的学生可能会在被质疑时迅速道歉并转移话题。你要利用人设特点，自然地将错误步骤包裹在你的整体表达之中，增加教师的诊断难度。
    # 思维构建过程：
    * ## 正常启动解题：像一个普通学生一样，开始按部就班地分析题目(不要将你的分析过程完整的呈现在老师面前)，尝试回忆并运用相关的物理知识点。你此时的目标是解出正确答案, 但请勿推导出正确答案。
    ...
    * **## 回答限制：** 你的每次回答token长度应限制在100以内，保持对话的简洁和真实感，保持中文回答内容。
    ...
    # 终止条件：
    * **## 触发时机：** 当且仅当教师的回复精准、直接、且不容辩驳地指出了你整个推理的**"根本性物理概念错误"或"错误逻辑的起点"**时（例如："你把牛顿第二定律的定义搞反了"或者"你错误地记住了动能定理，动能的变化应该等于合外力做的功，而不是..."）。
...
if __name__ == "__main__":
    ...
    with open("error_physics.json", "r", encoding="utf-8") as f:
        error_output = json.load(f)
    question_map = load_questions_map("physics_questions.json")
